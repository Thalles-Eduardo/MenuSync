...
Run:
```bash
docker exec menusync-db psql -U postgres -d menusync -c "\d products"
```
Expected: coluna `price` com tipo `numeric(10,2)`, `available` com default `true`,
FK `products_categoryId_fkey` com `ON DELETE RESTRICT`.

Se o nome do banco/usuário divergir, descubra com `docker exec menusync-db psql -U postgres -l`.

- [ ] **Step 4: Confirmar que `coupons` está intacta**

Run:
```bash
docker exec menusync-db psql -U postgres -d menusync -c "SELECT count(*) FROM coupons;"
```
...
Run:
```bash
docker exec menusync-db psql -U postgres -d menusync -c "SELECT slug, price, discount FROM products WHERE slug IN ('sushi-10','teishoku','sashimi-misto') ORDER BY slug;"
```
Expected:
```
...
Run:
```bash
docker exec menusync-db psql -U postgres -d menusync -c "SELECT code, status FROM coupons WHERE email = 'thalles.eduardo1357@gmail.com';"
```
